...
test.describe("site smoke", () => {
  test("homepage conversion path and key pages", async ({ page }) => {
    ...
    await page.getByRole("textbox", { name: /Phone/ }).fill("+13065505040");
    await page
      .getByRole("combobox", { name: "Primary Goal" })
      .selectOption("automations");
    await page
      .getByRole("textbox", { name: "Project Details" })
      .fill("Need help automating lead follow-up and reporting.");

    await page.getByRole("button", { name: "Send Project Details" }).click();
    await expect(page.getByText(/ready to send|has been sent/i)).toBeVisible();

    await page.goto("/portfolio");
    await expect(
      page.getByRole("heading", { name: "Implementation snapshots" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", {
